  ...
  for (var i in tree.children) {
    ...
    if (child.types.indexOf('object') !== -1) {
      ...
      if (child.children && utils.isObject(child.children)) {
        ...
        if (parent.required && parent.required.length > 0) {
          schema[i].items.required = Object.keys(props);
        }
      } else {
...
@return void
*/
Compiler.prototype.compile = function(tree, strict) {
  this.schema = {};
  ...
  if (tree.types.indexOf('object') !== -1) {
    ...
    if (tree.children && utils.isObject(tree.children)) {
      ...
      if (strict) {
        this.schema.items.required = Object.keys(tree.children);
        this.schema.items.additionalProperties = false;
      }
